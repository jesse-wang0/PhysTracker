...
def combine_images(input_path, output_path, threshold, force_flag):
    ...
    for i in range(0, len(img_files)-1):
        img1 = cv2.imread(f"{input_abs_path}/{img_files[i]}")
        img2 = cv2.imread(f"{input_abs_path}/{img_files[i+1]}")

        difference = cv2.absdiff(img1, img2)
        cv2.cvtColor(difference, cv2.COLOR_BGR2GRAY)
        cv2.imwrite(f"{output_abs_path}/diff{i}.png", difference)

    img = cv2.imread(f"{output_abs_path}/diff0.png", 0)
    ...
    for i, row in enumerate(img):
        for j, pixel in enumerate(row):
            if img[i][j] < 255:
                img[i][j] = 0
    cv2.imwrite(f"{output_abs_path}/final.png", img)
...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help = "Threshold value calculated by threshold tool."
    )
    parser.add_argument(
